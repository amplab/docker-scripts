...
function start_spark_master() {
    ...
    MASTER=$(sudo docker run -i -t -d -dns $NAMESERVER_IP -h master $VOLUME_MAP spark-master:$SPARK_VERSION)
...
function start_spark_workers() {
	...
	hostname="worker${i}"
	WORKER=$(sudo docker run -d -dns $NAMESERVER_IP -h $hostname $VOLUME_MAP spark-worker:${SPARK_VERSION} ${MASTER_IP} spark://${MASTER_IP}:7077)
